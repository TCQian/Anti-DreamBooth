...
def main(args):
    ...
    for _ in range(args.max_train_steps):
        with accelerator.accumulate(unet):
            # === Step 1: Forward pass (original PGD path) ===
            ...
            if args.target_image_path is not None:
                ...
                loss1 = loss1 - F.mse_loss(xtm1_pred, xtm1_target)

            # === Step 2: Backward for gradient mask ===
            accelerator.backward(loss1, retain_graph=True)
            grad_mask = pertubed_images.grad.clone()

            # === Step 3: DCT-guided frequency perturbation ===
            images_for_dct = ((pertubed_images + 1) / 2).clamp(0, 1)
            dct_perturbed = adaptive_frequency_perturbation(
                images_for_dct, grad_mask, alpha=args.pgd_alpha
            )
            ...
            loss2 = F.mse_loss(model_pred_dct.float(), target.float(), reduction="mean")
            combined_loss = loss1 + loss2

            # === Step 5: Final backward + PGD update ===
