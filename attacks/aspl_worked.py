...
def adaptive_frequency_perturbation(x, grad_mask, alpha=0.005):
    """
    x: image tensor in [0, 1], requires_grad=True
    grad_mask: image gradient w.r.t loss, shape (B, C, H, W)
    Returns perturbed image in [0, 1], still part of graph
    """
    dct = torch.fft.fft2(x)
    abs_dct = dct.abs()

    # Gradient-based saliency in frequency domain
    freq_mask = torch.fft.fft2(grad_mask).abs()
    # Normalise mask
    freq_mask = (freq_mask - freq_mask.min()) / (
        freq_mask.max() - freq_mask.min() + 1e-8
    )

    # Apply frequency-based perturbation
    dct_perturbed = dct + alpha * freq_mask * abs_dct
    perturbed = torch.fft.ifft2(dct_perturbed).real
...
def pgd_attack(
    ...
    for step in range(num_steps):
        ...
        # === Forward pass 1 (standard PGD path) ===
        latents = vae.encode(
        ...
        # === Gradient for frequency attack ===
        loss.backward(retain_graph=True)
        grad_mask = perturbed_images.grad.clone()

        # === DCT-guided frequency attack ===
        images_for_dct = ((perturbed_images + 1) / 2).clamp(0, 1)
        dct_perturbed = adaptive_frequency_perturbation(
            x=images_for_dct, grad_mask=grad_mask, alpha=alpha_dct
        )
        dct_perturbed = (dct_perturbed * 2 - 1).clamp(-1, 1)
        ...
        model_pred_dct = unet(
            noisy_latents_dct, timesteps, encoder_hidden_states
        ).sample

        loss_2 = F.mse_loss(model_pred_dct.float(), target.float(), reduction="mean")

        # === Combine and backprop once ===
        combined_loss = loss + loss_2
        unet.zero_grad()
        text_encoder.zero_grad()
        vae.zero_grad()
        perturbed_images.grad = None  # clear previous grads
